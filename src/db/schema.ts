...
export const BulletinTable = sqliteTable(
  "bulletins",
  {
    ...
    scope: text("scope", { enum: ["global", "site"] as const })
      .notNull()
      .default("global"),
    ...
    body: text("body"),
    requires_initials: integer("requires_initials", { mode: "boolean" }).notNull().default(false),
...
export const BulletinAcknowledgementTable = sqliteTable(
  ...
  (table) => ({
    bulletinAcknowledgementUnique: uniqueIndex(
      "bulletin_acknowledgements_bulletin_id_user_id_idx",
    ).on(table.bulletin_id, table.user_id),
  }),
);
...
export const bulletinAcknowledgementRelations = relations(
...
export type AttendanceSiblingParticipantEntity =
  typeof AttendanceSiblingParticipantTable.$inferSelect;
export type AttendanceSiblingParticipantInsert =
  typeof AttendanceSiblingParticipantTable.$inferInsert;
