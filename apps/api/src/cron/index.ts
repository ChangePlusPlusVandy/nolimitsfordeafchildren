...
export function initializeCronJobs(): void {
  ...
  cron.schedule("0 8 * * *", async () => {
    ...
    try {
      ...
      console.log(`[Cron] Birthday job completed: ${result.sent} notifications sent, ${result.errors} errors`);
    } catch (error) {
  ...
  cron.schedule("0 9 * * 1", async () => {
    ...
    try {
      ...
      console.log(`[Cron] Audiogram job completed: ${result.sent} reminders sent, ${result.errors} errors`);
    } catch (error) {
 ...
 */
export async function runJobManually(jobName: "birthday" | "audiogram"): Promise<{ sent: number; errors: number }> {
  switch (jobName) {
    case "birthday":
