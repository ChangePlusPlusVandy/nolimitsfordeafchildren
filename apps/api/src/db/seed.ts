...
const SCHOOLS = [
  "Sunshine Preschool", "Little Stars Daycare", "Happy Kids Academy", "Rainbow Learning Center",
  "ABC Preschool", "Tiny Tots Academy", "Bright Futures Preschool", "Creative Kids Center",
  "Lincoln Elementary", "Washington Elementary", "Jefferson Elementary", "Roosevelt Elementary",
  "Kennedy Elementary", "Madison Elementary", "Franklin Elementary", "Adams Elementary",
  "Oak Tree School", "Pine Valley Academy", "Maple Leaf School", "Cedar Grove Elementary",
  "Westside Elementary", "Eastside Elementary", "Northgate School", "Southpark Elementary",
  "Valley View School", "Hillside Academy", "Lakewood Elementary", "Riverside School",
  "Home Schooled", "Private Tutor", "Online Academy", "Montessori School",
];

const LANGUAGES = ["English", "Spanish", "Mandarin", "Korean", "Vietnamese", "Tagalog", "Armenian", "Farsi"];
...
const AGE_SPECIALTIES = ["infant", "toddler", "preschool", "elementary", "middle_school", "high_school", "young_adult", "all_ages"] as const;
...
const TOPICS = ["animals", "colors", "shapes", "family", "food", "weather", "body parts", "emotions", "transportation", "daily routines"];
...
async function seed() {
  ...
  const siblings = [];
  const relationships = ["brother", "sister", "twin brother", "twin sister", "half-brother", "half-sister"];
  ...
  const relationshipTypes = ["mother", "father", "grandmother", "grandfather", "guardian", "aunt", "uncle"];
  ...
  const absenceReasons = ["sick", "family_emergency", "transportation", "schedule_conflict", "no_show_unknown", "other"] as const;
  ...
  for (let i = 0; i < 300; i++) {
    ...
    attendance.push({
      ...
      reason_text: status !== "present" ? randomElement(["Had a cold", "Family event", "Car trouble", "Doctor appointment", null]) : null,
  ...
  for (let i = 0; i < 50; i++) {
    ...
    makeupRequests.push({
      ...
      reason_text: randomElement(["Child was sick", "Family emergency", "Transportation issue", "Scheduling conflict", null]),
      preferred_dates: randomElement(["Any Saturday morning", "Next week afternoon", "Flexible", "Weekday morning preferred", null]),
      status,
      requested_by: parentUsers.find(u => u.id === parent.user_id)!.id,
      reviewed_by: isReviewed ? adminUsers[0]!.id : null,
      review_notes: isReviewed ? randomElement(["Approved for next Saturday", "Scheduled with same teacher", "No availability, denied", "Completed successfully", null]) : null,
    });
  }
  ...
  for (let i = 0; i < 50; i++) {
    ...
    scheduleChangeRequests.push({
      ...
      requested_by: parentUsers.find(u => u.id === parent.user_id)!.id,
      reviewed_by: isReviewed ? adminUsers[0]!.id : null,
      review_notes: isReviewed ? randomElement(["Schedule change approved", "New slot available", "Cannot accommodate at this time", "Completed transfer", null]) : null,
    });
  }
