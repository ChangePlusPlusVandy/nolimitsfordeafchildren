...
import { cloneAssessment, createAssessment, listAssessmentsForStudent } from "@/client/assessments";
...
import { createNote, deleteNote, listNotesForStudent, updateNote } from "@/client/notes";
import { getStudentDetails, updateGuardianSummary } from "@/client/students";
...
export default function TeacherStudentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    queryKey: ["students", "show", id, "teacher-scope"],
  ...
  const {
    ...
    queryFn: () => listNotesForStudent(id, { page: 1, limit: 100 }),
    enabled: !!id && !!student,
  });
  ...
  const {
    ...
    queryFn: () => listAssessmentsForStudent(id, { page: 1, limit: 100 }),
    enabled: !!id && !!student,
  });

  const createNoteMutation = useMutation({
    mutationFn: (note: string) => createNote(id, note),
    onSuccess: () => {
  ...
  const updateNoteMutation = useMutation({
    mutationFn: ({ noteId, note }: { noteId: string; note: string }) => updateNote(noteId, note),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["session-notes", id, "teacher-scope"] });
      setNoteDialogOpen(false);
      setNoteText("");
  ...
  const updateGuardianSummaryMutation = useMutation({
    mutationFn: (guardianSummary: string) => updateGuardianSummary(id, guardianSummary),
    onSuccess: () => {
    ...
    return (
      <PageContainer>
        <PageHeader
          ...
          back="/my-day"
          breadcrumbs={[{ label: "My Day", href: "/my-day" }, { label: "Student Details" }]}
        />
        <ErrorAlert
          message={
                          ...
                          secondary={
                            <>
                              <Typography component="span" variant="body2">
                                Pre:{" "}
                                {cycle.pre_assessment ? `${cycle.pre_assessment.score}/20` : "-"}
                              </Typography>
                              <Typography component="span" variant="body2" sx={{ ml: 1.5 }}>
                                Post:{" "}
                                {cycle.post_assessment ? `${cycle.post_assessment.score}/20` : "-"}
                              </Typography>
                              ...
                              <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                                {cycle.pre_assessment && (
                                  <Button
                                    ...
                                    onClick={(event) => {
                                      ...
                                      if (!cycle.pre_assessment) return;
                                ...
                                {cycle.post_assessment && (
                                  <Button
                                    ...
                                    onClick={(event) => {
                                      event.preventDefault();
                                      if (!cycle.post_assessment) return;
                                      cloneAssessmentMutation.mutate({
                              ...
                              {(cycle.post_assessment?.focuses?.length ||
                                cycle.pre_assessment?.focuses?.length) && (
                                <Typography component="div" variant="body2" sx={{ mt: 0.5 }}>
                                  ...
                                  {(
                                    ...
                                    .map(
                                      (focus) =>
                                        `${focus.goal} (${focus.score}/${focus.max_score})`,
                                    )
                                    .join("; ")}
                                </Typography>
                              )}
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            fullWidth
            value={noteText}
            onChange={(event) => setNoteText((event.target as unknown as { value: string }).value)}
