...
import BulletinCard from "@/client/components/bulletin/BulletinCard";
import CreateBulletinModal from "@/client/components/bulletin/CreateBulletinModal";
import EmptyState from "@/client/components/EmptyState";
import ErrorAlert from "@/client/components/ErrorAlert";
import PageContainer from "@/client/components/PageContainer";
import PageHeader from "@/client/components/PageHeader";
import SectionCard from "@/client/components/SectionCard";
import { CardGridSkeleton } from "@/client/components/skeletons";
...
export default function BulletinBoardPage() {
  ...
  const { data: viewStats } = useQuery({
    queryKey: ["bulletins", "view-stats", selectedBulletinId],
    queryFn: () => {
      if (!selectedBulletinId) throw new Error("No bulletin selected");
      return getViewStats(selectedBulletinId);
    },
    enabled: isAdmin && !!selectedBulletinId,
  });

  const { data: acknowledgementStats } = useQuery({
    queryKey: ["bulletins", "ack-stats", selectedBulletinId],
    queryFn: () => {
      if (!selectedBulletinId) throw new Error("No bulletin selected");
      return getAcknowledgementStats(selectedBulletinId);
    },
    enabled: isAdmin && !!selectedBulletinId,
  });

  const acknowledgeMutation = useMutation({
    mutationFn: ({ bulletinId, initials }: { bulletinId: string; initials: string }) =>
      acknowledgeBulletin(bulletinId, { initials }),
    onSuccess: async () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["bulletins", "index", queryParams],
    queryFn: () => listBulletins(queryParams),
  });
  ...
  const handleBulletinClick = async (bulletin: Bulletin) => {
    try {
      const latest = await getBulletinDetails(bulletin.id);
  ...
  const handleAcknowledge = async () => {
    ...
    const latest = await getBulletinDetails(selectedBulletin.id);
    setSelectedBulletin(latest);
  };
            ...
            <DialogContent>
              ...
              {isAdmin && (
                <Box sx={{ mt: 3 }}>
                  ...
                  <Typography variant="subtitle2" gutterBottom>
                    Acknowledgements (
                    {acknowledgementStats?.count ?? selectedBulletin.acknowledgement_count ?? 0})
                  </Typography>
                ...
                <Box sx={{ mt: 3 }}>
                  ...
                  <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                    <TextField
                      ...
                      onChange={(event) =>
                        setAckInitials(
                          (event.target as unknown as { value: string }).value.toUpperCase(),
                        )
                      }
