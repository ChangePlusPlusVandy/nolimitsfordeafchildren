...
export default function UserDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { id: string } & Record<string, unknown>) => updateUser(data as never),
    onSuccess: () => {
  ...
  const { data: availableStudents } = useQuery({
    queryKey: ["users", "students-for-link", id],
    queryFn: () => getUserStudents({ page: 1, limit: 200 }),
  ...
  const linkStudentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users", "show", id] });
  ...
  const unlinkStudentMutation = useMutation({
    mutationFn: (studentId: string) => unlinkStudentFromUser({ userId: id, studentId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["users", "show", id] });
  ...
  const breadcrumbs = [{ label: "Users", href: "/users" }, { label: user?.name ?? "User Details" }];
  ...
  return (
    <PageContainer>
      ...
      {/* Link student dialog */}
      <Dialog
        open={linkStudentDialogOpen}
        onClose={() => setLinkStudentDialogOpen(false)}
        fullWidth
        maxWidth="sm"
      >
